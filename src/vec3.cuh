...
__device__ inline vec3 random_unit_vector(curandState *rand_state) {
    auto a = random_float(rand_state, 0, 2*pi);
    auto z = random_float(rand_state, -1, 1);
    auto r = sqrt(1 - z*z);
...
__device__ inline vec3 random_in_hemisphere(curandState *rand_state, const vec3& normal) {
